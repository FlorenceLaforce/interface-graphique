...
class Formulaire(tk.Tk):
    ...
    def init_widgets(self):
        ...
        self.Btn_Importer_Json = tk.Button(footer_content, text="Importer JSON")
        ...
        self.Btn_Exporter_Json = tk.Button(footer_content, text="Exporter JSON")
    ...
    def exporterJson(self):
        if not self.rows:
            ...
            return
        path = filedialog.asksaveasfilename(title="Exporter JSON", defaultextension=".json", filetypes=(("JSON files", "*.json"),))
        if not path:
            return
        try:
            ...
            messagebox.showerror("Exporter", str(e))
